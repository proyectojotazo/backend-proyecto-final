const multer = require("multer");
const path = require("path");
const { access, mkdir } = require("fs");
const { getUserFromJwt } = require("../utils");

// Obtener ID de usuario
const userId = (req) => {
  const jwtToken = req.get("Authorization").split(" ")[1];
  const usuarioId = getUserFromJwt(jwtToken).toString();
  return usuarioId;
};

// Storage de Multer
const imageStorage = multer.diskStorage({
  destination: (req, file, cb) => {
    const dest = "./public/upload-files/" + userId(req);
    access(dest, function (error) {
      if (error) {
        // console.log("Directorio no existe");
        return mkdir(dest, { recursive: true }, (err) => cb(err, dest));
      } else {
        // console.log("Directorio existe.");
        return cb(null, dest);
      }
    });
  },
  filename: (req, file, cb) => {
    if (file.fieldname === "avatar") {
      cb(
        null,
        file.fieldname + "_" + userId(req) + path.extname(file.originalname)
      );
    } else {
      // TODO: cambiar nombre de archivo para artículos
      cb(
        null,
        file.fieldname +
          "_" +
          Date.now() +
          "_" +
          userId(req) +
          path.extname(file.originalname)
      );
    }
  },
});

// Límite de tamaño del archivo subido
const limit = {
  fileSize: 2000000, // 2000000 Bytes = 2MB
};

// Filtro de archivo
const imageFilter = (req, file, cb) => {
  if (!file.originalname.match(/\.(png|jpg|jpeg|gif)$/)) {
    return cb(new Error("Sólo se admiten imágenes tipo: jpg, jpeg, png y gif"));
  }
  cb(null, true);
};

// Configuración para el upload de multer
const upload = multer({
  storage: imageStorage,
  limits: limit,
  fileFilter: imageFilter,
});

module.exports = upload;
